...
У пакета `editorconfig-checker` версия бинарника закреплена отдельно, переменной `EC_VERSION` в npm-скрипте `editorconfig`. `npm outdated` показывает только версию обёртки, бинарник поднимается руками.
...
- правим конфликты (если есть) в файле _package.json_;   
   - вложенный пункт со сломанным отступом;
