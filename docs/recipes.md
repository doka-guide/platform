...
**RSS-фид.** `/feed/index.xml` в формате Atom. Список материалов берётся не из коллекций, а из `CHANGELOG.md` репозитория контента: он скачивается с `raw.githubusercontent.com` на этапе сборки и разбирается в `src/libs/changelog-parser/`. В фид уходят 50 последних записей, у каждой — описание из фронтматтера и обложка: та же картинка, что в `og:image` материала. Описание берётся только у материалов, которые есть в сборке, поэтому при частичном `CONTENT_REP_FOLDERS` часть записей останется без текста — на боевой сборке подключён весь контент.

Обложка отдаётся полями Media RSS — `media:content` и `media:thumbnail`, — а не картинкой внутри `content`. Своего поля под обложку в Atom нет, и если положить картинку в содержимое записи, читалка покажет её дважды: своей карточкой и в теле. Полей два, потому что читалки смотрят разные: Feedly берёт `media:content`, часть остальных — `media:thumbnail`. Проверять правки фида нужно в живой читалке: по разметке этого не видно.
