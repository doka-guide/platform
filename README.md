...
Добавьте свои тесты. Для этого либо создайте файл с окончанием `.test.js` рядом с файлом, функциональность из которого вы хотите протестировать, либо создайте папку `__test__` и добавляйте файлы тестов в нее. Тесты внутри этой папки могут иметь любое название и не обязательно должны заканчиваться на `.test.js`.
